...
export default class DataService {
  ...
  //   let finalAttachmentUrl: string | undefined; // This will hold the URL after successful upload

  //   // --- NEW: Handle attachment upload first ---
  //   if (attachmentFile) {
  //     console.log("Attempting to upload attachment file:", attachmentFile.name);
  //     const formData = new FormData();
  //     formData.append('file', attachmentFile); // 'file' must match the name expected by your API route

  //     try {
  //       const uploadResponse = await fetch('/api/upload', { // Call your Next.js API route
  //         method: 'POST',
  //         body: formData,
  ...
  //         throw new Error(
  //           `Failed to upload attachment: ${uploadResponse.statusText} - ${errorData.message || 'Unknown error'}`
  //         );
  //       }

  //       const uploadResult = await uploadResponse.json();
  //       finalAttachmentUrl = uploadResult.fileUrl; // Get the URL from your API response
  //       console.log("Attachment uploaded successfully, URL:", finalAttachmentUrl);

  //     } catch (uploadError) {
  //       console.error("Error during file upload process:", uploadError);
  //       throw new Error(`Failed to process attachment upload: ${(uploadError as Error).message}`);
  //     }
  //   }
  //   const newExcuseData = {
