...
export default function StudentFinesPage() {
  ...
  const studentId = idParam ? Number(idParam) : undefined;
  ...
  const fetchFinesData = useCallback(async () => {
    ...
    if (typeof studentId === "number" && !isNaN(studentId)) {
      ...
      try {
        ...
        const allPenalties = await DataService.getPenaltiesForStudent(studentId);
        console.log("fiiii", allPenalties)
        ...
        console.log("fiiii", currentUnpaid)
  ...
  }, [studentId, t, isMounted]); // Dependencies for useCallback
