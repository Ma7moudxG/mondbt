...
interface FinesPageProps {
  params: {
    parentId: string; // Next.js dynamic routes provide params as strings (matching folder name)
  };
}

const FinesPage: React.FC<FinesPageProps> = ({ params }) => {
  const parentId = Number(params.parentId); // Convert the string param to a number
  const [totalFines, setTotalFines] = useState<number>(0);
  const [selectedPaymentMethod, setSelectedPaymentMethod] =
    useState<string>("credit_card"); // Default to credit card

  useEffect(() => {
    if (!isNaN(parentId)) { // Check if parentId is a valid number
      // --- DEBUGGING LOGS START ---
      console.log("FinesPage (Payment Page): useEffect triggered for parentId:", parentId);
      // --- DEBUGGING LOGS END ---

      // Fetch the total penalties for this parent (all their kids)
      const fines = DataService.getTotalPenaltiesForParent(parentId);

      // --- DEBUGGING LOGS START ---
      console.log("FinesPage (Payment Page): Fines fetched from DataService:", fines);
      // --- DEBUGGING LOGS END ---
      setTotalFines(fines);
    } else {
      setTotalFines(0); // Reset if parentId is not valid
    }
  }, [parentId]); // Re-run when parentId changes
  ...
  const handlePayNowClick = () => {
    if (selectedPaymentMethod) {
      ...
      alert(
        `Initiating mock payment for ${totalFines} SAR using ${selectedPaymentMethod} for parent ID: ${parentId}.`
      );
  ...
  // Display an error or loading message if parentId is invalid
  if (isNaN(parentId)) {
    return <div className="text-center py-8">Invalid Parent ID</div>;
  }
