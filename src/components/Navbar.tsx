...
export default function Header() {
  ...
  const router = useRouter(); // Effect to set isMounted and read client-side storage

  useEffect(() => {
    setIsMounted(true); // Component has mounted on the client // Read client-side storage only after mount
    ...
    if (savedLang) {
      ...
      // Ensure localStorage is available and it's client-side
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex space-x-2">
          ...
          <button
            ...
            className={`px-3 py-1 text-xs rounded-full border ${
              useHijri
                ? "bg-[#8447AB] text-white border-[#8447AB] hover:bg-white hover:text-[#8447AB]"
